...
{
    public partial class RDEngineerwork : System.Web.UI.Page
    {
        ...
        private void Showworkdepartment()
        {
            DBConnect dbCenter = new DBConnect("Center");
            string sqlBranch = "SELECT * FROM Workdepartment";
            DataSet dsBranch = dbCenter.DBSelect(sqlBranch);
            int countBranch = dsBranch.Tables[0].Rows.Count;
            ddlworkdepartment.Items.Clear();
            if (countBranch > 0)
            {
                for (int i = 0; i < countBranch; i++)
                {
                    if (i == 0)
                    {
                        ...
                        ddlworkdepartment.Items.Add(new ListItem("ทั้งหมด", "0"));
                    }
                    string wkdepartment_id = dsBranch.Tables[0].Rows[i]["wkdepartment_id"].ToString();
                    string wkdepartment_name = dsBranch.Tables[0].Rows[i]["wkdepartment_name"].ToString();

                    ddlworkdepartment.Items.Add(new ListItem(wkdepartment_name, wkdepartment_id));
                }
            }
        }
